...
class Embedding1D(nn.Module):
    ...
    def forward(self, B):
        ...
        return emb.unsqueeze(0).repeat(B, 1)
...
def gen_pos_2d(x):
    B, H, W, C = x.shape
    row_indices = torch.arange(H).unsqueeze(1).repeat(1, W).unsqueeze(-1)
    col_indices = torch.arange(W).unsqueeze(0).repeat(H, 1).unsqueeze(-1)
