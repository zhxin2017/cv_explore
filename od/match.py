...
def assign_query(boxes_gt, boxes_pred, cids_gt, cls_pred, gt_pos_mask):
    ...
    iouloss = torchvision.ops.distance_box_iou_loss(boxes_pred, boxes_gt)
    ...
    cls_loss = nn.CrossEntropyLoss(reduction='none')(cls_pred, cids_gt).view(B, N, N)

    total_loss = (iouloss * 10 + cls_loss) * gt_pos_mask + 10000.0 * (1 - gt_pos_mask)
    total_loss[total_loss == torch.nan] = 10000.0
