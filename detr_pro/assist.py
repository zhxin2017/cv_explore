...
def separate_bgd_grids(w, h, x_shift, y_shift, boxes):
    ...
    grid_x1 = ((torch.arange(w, device=boxes.device) + x_shift) * grid_size).view(w, 1).repeat(1, h).reshape(n_grid)
    grid_y1 = ((torch.arange(h, device=boxes.device) + y_shift) * grid_size).view(1, h).repeat(w, 1).reshape(n_grid)
    grid_x2 = grid_x1 + grid_size
    grid_y2 = grid_y1 + grid_size
    grid_box = torch.concat([grid_x1, grid_y1, grid_x2, grid_y2], dim=-1)
