...
from common.config import train_annotation_file, train_img_od_dict_file, max_img_size, \
...
def train(epoch, batch_size, population, num_sample, weight_recover=0.5, gamma=4):
    ds = detr_pro_dataset.DetrProDS(train_img_dir, dicts, sample_num=population, random_flip=True)
    ...
    for i in range(epoch):
        for j, (img_batch, masks_batch, boxes_gt_xyxy_batch, cids_gt_batch, img_id_batch) in enumerate(dl):
            ...
            loss = cls_pos_loss_batch + cls_neg_loss_batch + box_loss_batch + src_cls_loss
            optimizer.zero_grad()
            t = time.time()
            loss.backward()
            t_bp = time.time() - t
            ...
            print(f'smp {num_sample}|epoch {i + 1}/{epoch}|batch {j}|'
                  f'posl {cls_pos_loss_batch.detach().item() * 1000:.3f}|'
                  f'negl {cls_neg_loss_batch.detach().item() * 1000:.3f}|'
                  f'srcl {src_cls_loss.detach().item() * 1000:.3f}|'
                  f'bl {box_loss_batch.detach().item():.3f}|'
                  f'src_rc {src_cls_recall:.3f}|'
                  f'src_ac {src_cls_accu:.3f}|'
                  f'ac {accu:.3f}|rc {recall:.3f}: {tp}/{n_pos_batch}|'
